        ...
        child: PrimaryButton(
            onPressed: () {
              if (isFromLocation == true) {
                context
                    .read<LocationBloc>()
                    .add(ApplyAssetsListFilter(filterMap: assetsFilterMap));
                Navigator.pop(context);
                context.read<LocationBloc>().add(FetchLocationDetails(
                    locationId: expenseId, selectedTabIndex: 7));
              } else {
                context
                    .read<AssetsBloc>()
                    .add(ApplyAssetsFilter(assetsFilterMap: assetsFilterMap));
                context.read<AssetsBloc>().assetsDatum = [];
                context.read<AssetsBloc>().hasReachedMax = false;
